...
extension SettingsViewController: UITableViewDelegate {
  ...
  @objc func navigateToCalenroPro() {
      ...
      // Obtén las ofertas configuradas en RevenueCat
      Purchases.shared.getOfferings { (offerings, error) in
          if let error = error {
              print("Error obteniendo las ofertas: \(error.localizedDescription)")
              return
          }

          if let offering = offerings?.current {
              // Asegúrate de que exista un paquete en la oferta
              if let package = offering.availablePackages.first {
                  // Presenta la compra (puedes personalizar tu propia paywall aquí)
                  self.showPaywall(for: package)
              } else {
                  print("No hay paquetes disponibles en la oferta actual.")
              }
          }
      }
  }

  // Función para mostrar tu paywall personalizada
  func showPaywall(for package: Package) {
  ...
  func purchasePackage(_ package: Package) {
      Purchases.shared.purchase(package: package) { (transaction, purchaserInfo, error, userCancelled) in
          if let error = error {
              print("Error al realizar la compra: \(error.localizedDescription)")
              return
          }
          ...
          // Verifica si el usuario ahora es Pro
          if purchaserInfo?.entitlements["pro"]?.isActive == true {
              print("¡Compra exitosa! El usuario ahora tiene acceso Pro.")
          }
      }
  }
